...
export default function PreviewPage() {
  ...
  const transfers = [{ id: "00000000-0000-4000-8000-000000000501", status: "posted", amount_minor: 150000, currency: "USD", occurred_at: "2026-09-04", created_at: at("2026-09-04", "16:00"), effective_sequence: "6", title: "转入券商准备投资", source_account_kind: "bank", destination_account_kind: "brokerage" }];
  const accounts = [{ kind: "bank", name: "共同银行" }, { kind: "brokerage", name: "共同券商" }];
  const accountBalances = accountCashBalances(entries.map(ledgerEventFromRow), transfers.map(cashTransferFromRow));
  const activity = [...entries, ...transfers.map((transfer) => ({ ...transfer, entry_type: "account_transfer" }))].sort((left, right) => Number(right.effective_sequence ?? 0) - Number(left.effective_sequence ?? 0));
  const proposals = [
  ...
  return <PreviewClient entries={activity} investmentEntries={entries} proposals={proposals} investments={investments} valuations={valuations} members={members} accounts={accounts} accountBalances={accountBalances}/>;
}
