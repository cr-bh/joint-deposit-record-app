...
export default function AppClient({ household, userId, role, entries, proposals, investments, valuations, members, ledgerSummary, postedEntryCount, investmentSnapshots }: Props) {
  ...
  const [tab, setTab] = useState("总览"); const [recordOpen, setRecordOpen] = useState(false); const [investmentOpen, setInvestmentOpen] = useState(false); const [investmentActionId, setInvestmentActionId] = useState<string | null>(null); const [inviteOpen, setInviteOpen] = useState(false); const [message, setMessage] = useState("");
