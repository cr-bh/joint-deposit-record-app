...
import type { AccountBalances } from "@/lib/domain/account-balances";
...
type Props = { household: { id: string; name: string; reportingCurrency: string; ledgerVersion?: number }; userId: string; role: string; entries: Row[]; recentEntries?: Row[]; proposals: Row[]; investments: Row[]; valuations: Row[]; members: Row[]; accounts?: Row[]; accountBalances?: AccountBalances; ledgerSummary?: LedgerSummary; postedEntryCount?: number; initialTab?: string; ledgerPage?: number; ledgerPageCount?: number; investmentSnapshots?: Record<string, InvestmentSnapshot> };
...
export default function AppClient({ household, userId, role, entries, recentEntries = entries, proposals, investments, valuations, members, accounts = [], accountBalances, ledgerSummary, postedEntryCount, initialTab = "总览", ledgerPage = 1, ledgerPageCount = 1, investmentSnapshots }: Props) {
  const router = useRouter();
  const [tab, setTab] = useState(initialTab); const [recordOpen, setRecordOpen] = useState(false); const [investmentOpen, setInvestmentOpen] = useState(false); const [investmentActionId, setInvestmentActionId] = useState<string | null>(null); const [inviteOpen, setInviteOpen] = useState(false); const [message, setMessage] = useState("");
...
function Approvals({ proposals, userId, decide, currency, members }: { proposals: Row[]; userId: string; decide: (id: string, approve: boolean) => void; currency: string; members: Row[] }) {
  return <div className="space-y-3">{proposals.length ? proposals.map((proposal) => {
    ...
    return <article key={String(proposal.id)} className="rounded-2xl border bg-white p-5"><div className="flex justify-between gap-3"><div><b>{String(payload.title)}</b><p className="mt-1 text-xs text-gray-500">{transfer ? "账户内部划转" : String(payload.type)} · {Number(payload.amountMinor) ? money(Number(payload.amountMinor), String(payload.currency || currency)) : "非现金操作"}</p>{transfer ? <p className="mt-1 text-xs text-gray-500">{source} → {destination}</p> : ["deposit", "expense", "expense_refund", "settlement"].includes(String(payload.type)) && <p className="mt-1 text-xs text-gray-500">入账账户：{account}</p>}{payer && <p className="mt-1 text-xs text-gray-500">实际付款人：{payer}</p>}</div><span className="rounded-full bg-amber-100 px-2 py-1 text-xs">{String(proposal.status)}</span></div>{["pending_approval", "overdue_pending"].includes(String(proposal.status)) && <div className="mt-4">{mine ? <span className="text-xs text-gray-500">你不能审批自己提交的记录。</span> : <><button onClick={() => decide(String(proposal.id), false)} className="mr-2 rounded-lg border px-3 py-2 text-sm">驳回</button><button onClick={() => decide(String(proposal.id), true)} className="rounded-lg bg-[#1f5243] px-3 py-2 text-sm text-white">批准</button></>}</div>}</article>;
  }) : <Empty text="暂无审批记录。"/>}</div>;
}
export function Ledger({ entries, currency, members, accounts = [], totalEntries, page = 1, pageCount = 1 }: { entries: Row[]; currency: string; members: Row[]; accounts?: Row[]; totalEntries: number; page?: number; pageCount?: number }) {
  return <div className="overflow-hidden rounded-2xl border bg-white">{totalEntries > 0 && <p className="border-b bg-[#f7f6f1] px-4 py-3 text-xs text-gray-600">第 {page} / {pageCount} 页 · 本页 {entries.length} 笔，共 {totalEntries} 笔；总览与投资持仓始终按全部已入账记录计算。</p>}{entries.length ? entries.map((entry) => {
    ...
    const transfer = entry.entry_type === "account_transfer";
    ...
    return <div key={String(entry.id)} className="flex justify-between gap-3 border-b p-4"><div><b>{String(entry.title)}</b><p className="mt-1 text-xs text-gray-500">{String(entry.occurred_at)} · {transfer ? "账户内部划转" : String(entry.entry_type)}{entry.category ? ` · ${String(entry.category)}` : ""}</p>{transfer ? <p className="mt-1 text-xs text-gray-500">{accountName(accounts, source)} → {accountName(accounts, destination)}</p> : kind && <p className="mt-1 text-xs text-gray-500">入账账户：{accountName(accounts, kind)}</p>}{payer && <p className="mt-1 text-xs text-gray-500">实际付款人：{payer}</p>}</div><b>{money(Number(entry.amount_minor), String(entry.currency || currency))}</b></div>;
  }) : <Empty text="暂无已入账流水。"/>}{pageCount > 1 && <nav aria-label="流水分页" className="flex items-center justify-between border-t bg-[#f7f6f1] p-3"><PaginationLink page={page - 1} disabled={page <= 1}>上一页</PaginationLink><span className="text-xs text-gray-600">第 {page} / {pageCount} 页</span><PaginationLink page={page + 1} disabled={page >= pageCount}>下一页</PaginationLink></nav>}</div>;
}
function PaginationLink({ page, disabled, children }: { page: number; disabled: boolean; children: string }) {
  ...
  return disabled ? <span aria-disabled="true" className={className}>{children}</span> : <Link className={className} href={`/app?tab=ledger&ledgerPage=${page}`} prefetch={false}>{children}</Link>;
}
...
export function RecordModal({ close, household, investments, members, userId, refresh, setMessage, readOnly = false, mode = "general", initialInvestmentId }: { close: () => void; household: Props["household"]; investments: Row[]; members: Row[]; userId: string; refresh: () => void; setMessage: (value: string) => void; readOnly?: boolean; mode?: "general" | "investment"; initialInvestmentId?: string }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      const payload = {
        ...
        sourceAccountKind: type === "account_transfer" ? sourceAccountKind : undefined,
        destinationAccountKind: type === "account_transfer" ? (sourceAccountKind === "bank" ? "brokerage" : "bank") : undefined,
